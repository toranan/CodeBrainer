...
| `user_id` | VARCHAR(36) | NOT NULL | 사용자 ID (UUID 형식) |
| `problem_id` | BIGINT | FK → problems(id) ON DELETE CASCADE | 문제 ID |
...
- `idx_submissions_user_problem` on `(user_id, problem_id)`

**채점 작업 상태 (status):**
- `QUEUED`: 채점 큐에 등록됨 (대기 중)
- `RUNNING`: 채점 실행 중
- `COMPLETED`: 채점 완료 (결과는 submission_results 참고)
- `FAILED`: 채점 시스템 오류

**중요:** `status`는 채점 **작업의 진행 상태**입니다. 실제 채점 결과(정답/오답)는 `submission_results` 테이블의 `tests_json`에서 `verdict` 필드를 확인하세요.

**테스트케이스별 채점 결과 (verdict):**
- `AC`: Accepted (정답)
- `WA`: Wrong Answer (오답)
...
sequenceDiagram
    ...
    Orchestrator->>DB: Submission 레코드 생성 (QUEUED)
    Orchestrator-->>Client: submissionId 반환

    Orchestrator->>RabbitMQ: 채점 요청 메시지 발행
    Orchestrator->>DB: Submission 상태 업데이트 (RUNNING)
    RabbitMQ->>Judge0: 테스트케이스별 실행 요청

    Judge0-->>RabbitMQ: 채점 결과 반환
    RabbitMQ->>Orchestrator: 결과 처리
    Orchestrator->>DB: SubmissionResult 저장 (verdict: AC/WA/TLE/RE)
    Orchestrator->>DB: Submission 상태 업데이트 (COMPLETED/FAILED)
```
...
**backend/docker-compose.yml 사용**

```yaml
spring:
  datasource:
    url: jdbc:postgresql://postgres:5432/codebrainer
    username: codebrainer
    password: codebrainer

judge0:
  api-url: http://3.27.95.194:2358  # EC2
...
**환경 변수 설정 (Vercel/AWS 등)**

```bash
# Database (Supabase)
DATABASE_URL=jdbc:postgresql://aws-1-ap-southeast-1.pooler.supabase.com:5432/postgres
DATABASE_USERNAME=postgres.sqwobsmtrgjuhgymfwtl
DATABASE_PASSWORD=your-password

# Judge0 (EC2)
JUDGE0_API_URL=http://3.27.95.194:2358

# RabbitMQ (CloudAMQP 등)
...
STORAGE_BASE_PATH=/var/codebrainer/storage
```
