...
export default async function TossProblemsPage() {
  const problems = await fetchProblemSummaries()
  const tossProblems = problems.filter((p) => p.categories.includes("토스"))
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 py-10">
      <header className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-semibold text-slate-900">토스 기출 문제</h1>
          <Badge className="bg-indigo-100 text-indigo-800">TOSS</Badge>
        </div>
        <p className="text-sm text-slate-600">
          토스 NEXT 개발자 채용 코딩테스트 기출문제입니다.
        </p>
      </header>

      <div className="space-y-4">
        {tossProblems.length === 0 ? (
          <Card>
            <CardContent className="py-10 text-center text-sm text-slate-500">
              아직 등록된 토스 기출 문제가 없습니다.
            </CardContent>
          </Card>
        ) : (
          tossProblems.map((problem) => (
