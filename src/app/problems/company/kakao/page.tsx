...
export default async function KakaoProblemsPage() {
  const problems = await fetchProblemSummaries()
  const kakaoProblems = problems.filter((p) => p.categories.includes("카카오"))
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 py-10">
      <header className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-semibold text-slate-900">카카오 기출 문제</h1>
          <Badge className="bg-yellow-100 text-yellow-800">KAKAO</Badge>
        </div>
      ...
      <div className="space-y-4">
        {kakaoProblems.length === 0 ? (
          <Card>
            <CardContent className="py-10 text-center text-sm text-slate-500">
              아직 등록된 카카오 기출 문제가 없습니다.
            </CardContent>
          </Card>
        ) : (
          kakaoProblems.map((problem) => (
