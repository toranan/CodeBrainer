...
export default async function GoldProblemsPage() {
  const problems = await fetchProblemSummaries()
  const goldProblems = problems.filter((p) => p.difficulty === "GOLD")
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 py-10">
      <header className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <img src="/tiers/Gold.png" alt="골드" className="w-12 h-12 object-contain" />
          <h1 className="text-3xl font-semibold text-slate-900">골드 문제</h1>
          <Badge className="bg-yellow-100 text-yellow-700">GOLD</Badge>
        </div>
        <p className="text-sm text-slate-600">
          고급 수준의 알고리즘 문제입니다. 다양한 알고리즘 기법과 최적화가 필요합니다.
        </p>
      </header>

      <div className="space-y-4">
        {goldProblems.length === 0 ? (
          <Card>
