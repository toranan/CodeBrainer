...
export default async function PlatinumProblemsPage() {
  const problems = await fetchProblemSummaries()
  const platinumProblems = problems.filter((p) => p.difficulty === "PLATINUM")
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 py-10">
      <header className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <img src="/tiers/Platinum.png" alt="플래티넘" className="w-12 h-12 object-contain" />
          <h1 className="text-3xl font-semibold text-slate-900">플래티넘 문제</h1>
          <Badge className="bg-emerald-100 text-emerald-700">PLATINUM</Badge>
        </div>
        <p className="text-sm text-slate-600">
          최고 난이도의 알고리즘 문제입니다. 심화된 알고리즘 지식과 창의적인 접근이 필요합니다.
        </p>
      </header>

      <div className="space-y-4">
        {platinumProblems.length === 0 ? (
          <Card>
            <CardContent className="py-10 text-center text-sm text-slate-500">
              아직 등록된 플래티넘 문제가 없습니다.
            </CardContent>
          </Card>
        ) : (
          platinumProblems.map((problem) => (
